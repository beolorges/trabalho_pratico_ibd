...
# VERIFICA A EXISTENCIA DE UM BAIRRO NO BANCO E 
# SE NÃO EXISTIR, INSERE NO BANCO, RETORNANDO O ID
def retorna_id_bairro(bairro: str, cursor) -> int:
    SQL = f'''
        SELECT id
        FROM bairro b
        WHERE unaccent(b.nome) ilike unaccent('{bairro}');
    '''
    cursor.execute(SQL)
    res = cursor.fetchone()
...
# VERIFICA A EXISTENCIA DE UMA REGIAO NO BANCO E 
# SE NÃO EXISTIR, INSERE NO BANCO, RETORNANDO O ID
def retorna_id_regiao(regiao: str, cursor) -> int:
    SQL = f'''
        SELECT id
        FROM regiao b
        WHERE unaccent(b.nome) ilike unaccent('{regiao}');
    '''
    cursor.execute(SQL)
    res = cursor.fetchone()
...
if __name__ == '__main__':
    ...
    i = 0
    for bairro, regional in BAIRROS_REGIONAIS_BH:
        id_bairro = retorna_id_bairro(bairro, cursor)
        id_regiao = retorna_id_regiao(regional, cursor)

        SQL = f'''
            INSERT INTO bairro_regiao(id_bairro, id_regiao)
            VALUES ('{id_bairro}', '{id_regiao}')
        '''
        cursor.execute(SQL)
        i = i + 1
        print(f'Número de inserções: {i}')
        print(f'{bairro} : {regional}\n')
